...
!--------ML--------------------
   REAL(wp) ::   rspi60        =  4.0_wp   ! Kantha-Clayson 2004 for q^2*l equation, Harcourt 2013, 2015 gives 6-7
!-------ML--------------------- 
...
!ML rpsi6 is added, we don't know value for - sign=-1
   REAL(wp) ::   rpsi3m, rpsi3p, rpp, rmm, rnn, rpsi6!, rpsi60    !     -           -           -        -
!ML   !
...
!ML p_sst added, shear-Stockes production tern
!  SUBROUTINE zdf_gls( kt, p_sh2, p_avm, p_avt )
   SUBROUTINE zdf_gls( kt, p_sh2, p_shst, p_avm, p_avt )
!ML
      !!----------------------------------------------------------------------
      !!                   ***  ROUTINE zdf_gls  ***
      !!
...
! ML
      REAL(wp), DIMENSION(:,:,:), INTENT(inout   ) ::   p_sh2, p_shst  ! shear and shear-stokes production terms
! ML
...
! MLs
      REAL(wp) ::   prod, buoy, diss, zdiss, sm , zdirs,infinity !   -      -
! MLe
      ...
      p_shst(:,:,:) = 0._wp   ! NB - CANCEL STOKE EFFECT
...
! MLs               !
               zdir  = 0.5_wp + SIGN( 0.5_wp, buoy )   !        zdir =1(=0) if buoy >0(<0)
!               zdirs = 0.5_wp + SIGN( 0.5_wp, p_shst(ji,jj,jk) ) 
               !
...
!                zesh2 =   p_sh2(ji,jj,jk) + zdir*buoy + zdirs*p_shst(ji,jj,jk)                       ! production term
!                zdiss = ( diss - (1._wp-zdir)*buoy - (1._wp-zdirs)*p_shst(ji,jj,jk) ) / en(ji,jj,jk) ! dissipation term
                zesh2 =   p_sh2(ji,jj,jk) + zdir*buoy
                zdiss = ( diss - (1._wp-zdir)*buoy ) / en(ji,jj,jk)
!!gm
! MLe
               !
...
!------------------ MLs               !
             ...
             !  prod =  zratio *(rpsi1*p_sh2(ji,jj,jk)+rpsi6*p_shst(ji,jj,jk) ) ! shear and Stokes production
               ! there is an issue here: zratio * rpsi1 * p_sh2 works while rpsi1 * p_sh2 * zratio blows up
               prod =  zratio * rpsi1 * p_sh2(ji,jj,jk) + zratio * rpsi6 * p_shst(ji,jj,jk)
               !
!------------------- MLs
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namzdf_gls in configuration namelist' )
      ...
      CASE( 0 )                              ! k-kl  (Mellor-Yamada)
         ...
         rpsi2   = 0.5_wp
!------ML------
         rpsi6   = rspi60   ! from Kantha and Clayson, 2004, Harourt, 2013, 2015 suppose 6-7
!-----ML
         !
         SELECT CASE ( nn_stab_func )
      ...
      CASE( 1 )                              ! k-eps
         ...
         rpsi2   =  1.92_wp
!---------ML-----
         rpsi6   =  ( 2._wp+3._wp-rspi60)/1._wp ! conversion of q^2 *l constant to eps 
!--------=ML-----
         SELECT CASE ( nn_stab_func )
      ...
      CASE( 2 )                              ! k-omega
         ...
         rpsi2   =  0.833_wp
!-------------ML
         rpsi6   =  0._wp    !' ==>> require redifinnition'
!-------------ML         !
         SELECT CASE ( nn_stab_func )
      ...
      CASE( 3 )                              ! generic
         ...
         rpsi2   = 1.22_wp
!---------ML-----------
         rpsi6   = 0._wp    !'==>> require redefinition'
!---------ML--------
         SELECT CASE ( nn_stab_func )
