MODULE sbcmod
   ...
   !! $Id: sbcmod.F90 11536 2019-09-11 13:54:18Z smasson $
...
CONTAINS
   ...
   SUBROUTINE sbc_init
      ...
      !--- NB -------
         &             ln_st_swell, ln_compute_st, ln_d_st_dz,                       &
      !--- END NB ---
...
!!gm  lk_oasis is controlled by key_oasis3  ===>>>  It shoud be removed from the namelist 
      ...
      !--- NB -------
         WRITE(numout,*) '               compute swell stokes drift           ln_st_swell   = ', ln_st_swell
         WRITE(numout,*) '                  ATTENTION: the original formulation need to compute wind sea to not double count'
         WRITE(numout,*) '               compute stokes drift (monochromatic) ln_compute_st = ', ln_compute_st
         WRITE(numout,*) '               compute d/dz stokes drift analytic   ln_d_st_dz    = ', ln_d_st_dz
      !--- END NB ---
      ENDIF
      !
      IF( .NOT.ln_wave ) THEN
