...
EMODNET_RAW_cube = iris.load(
    "{}/EMODNET_v2020_NO_REPEAT_LAT_LON.nc".format(args.INCUBE_DIR[0])
)[0]
...
while SECTION < div + 1:
    ...
    values, lat_idx = find_nearest(
        EMODNET_RAW_cube.coord("latitude").points[:], np.min(lats)
    )
    lat_min_idx = lat_idx - 500  # ( for safety for fill interp)

    values, lon_idx = find_nearest(
        EMODNET_RAW_cube.coord("longitude").points[:], np.min(lons)
    )
    lon_min_idx = lon_idx - 500  # ( for safety for fill interp)

    # Max
    values, lat_idx = find_nearest(
        EMODNET_RAW_cube.coord("latitude").points[:], np.max(lats)
    )
    lat_max_idx = lat_idx + 500  # (for safety for fill interp)

    values, lon_idx = find_nearest(
        EMODNET_RAW_cube.coord("longitude").points[:], np.max(lons)
    )
    lon_max_idx = lon_idx + 500  # ( for safety for fill interp)

    print("\n SECTION {} of {}\n".format(SECTION, div))
    subsection_emodnet_raw_cube = EMODNET_RAW_cube[
    ...
    iris.save(
        subsection_emodnet_raw_cube.regrid(
    ...
    print("About to Nan")
    with ProgressBar():
        subsection_emodnet_raw_cube.data[
            subsection_emodnet_raw_cube.data > 1.0e33
        ] = np.nan
    ...
    with ProgressBar():
        iris.save(
            ...
            "{}/SUBSECTION/{:05d}_FILL_SUBSECTION_CUBE.nc".format(
...
] = EXTRAPOLATE_EMODNET_ON_EXPANDAMM15.sea_floor_depth_below_geoid[
    EXTRA_LAT:-EXTRA_LAT, EXTRA_LON:-EXTRA_LON
]


with ProgressBar():
    MASK_EMODNET_ON_EXPANDAMM15.to_netcdf(
        "{}/MASK_EMODNET_vDec2020_ON_EXPAND_AMM15.nc".format(args.OUT_DIR[0])
    )
with ProgressBar():
    EXTRAPOLATE_EMODNET_ON_EXPANDAMM15.to_netcdf(
        "{}/EXTRAPOLATE_EMODNET_vDec2020_ON_EXPAND_AMM15.nc".format(args.OUT_DIR[0])
    )
with ProgressBar():
    MASK_EXTRAPOLATE.to_netcdf(
        "{}/MASK_EXTRAPOLATE_EMODNET_vDec2020_ON_EXPAND_AMM15.nc".format(
            args.OUT_DIR[0]
        )
    )
