...
We read in the two surge dat sets, CS20 , CS3X interpolate each to the AMM15 grid
and combine into one data set. CS3X goes out to the old AMM7 bdy limits CS20
only as far the shelf break
Parameters
----------
IN_DIR : str
...
def set_history(cube):
    "Tags history of output netcdf creation to the file itself"
...
AMP_CS3X[AMP_CS3X < -1.0e33] = np.nan
AMP_CS3X[AMP_CS3X > 1.0e33] = np.nan
...
latitude_cs3x = DimCoord(
    lat_CS3X, standard_name="latitude", units="degrees", coord_system=cs
)
longitude_cs3x = DimCoord(
    lon_CS3X, standard_name="longitude", units="degrees", coord_system=cs
)

latitude_cs20 = DimCoord(
    lat_CS20, standard_name="latitude", units="degrees", coord_system=cs
)
longitude_cs20 = DimCoord(
    lon_CS20, standard_name="longitude", units="degrees", coord_system=cs
)
...
rotated_CS20_amm15 = CS20_cube.regrid(
    AMM15_cube, iris.analysis.Linear(extrapolation_mode="mask")
)
...
COMBINE_C3X_CS20.data[
    np.where(rotated_CS20_amm15.data[:] < 1000.0)
] = rotated_CS20_amm15.data[np.where(rotated_CS20_amm15.data[:] < 1000)]
