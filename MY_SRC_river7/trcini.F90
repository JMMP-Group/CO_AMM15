...
9000  FORMAT('      tracer nb : ',i2,'      name :',a10,'      initial content :',e18.10)
   ...
   SUBROUTINE trc_ini_sms
      ...
      IF(lwp) THEN                   ! control print
         ...
         DO jn = 1, jptra
            WRITE(numout,9001) jn, TRIM(ctrcnm(jn)), ln_trc_ini(jn), ln_trc_sbc(jn),ln_trc_cbc(jn),ln_trc_obc(jn), MAXVAL( trn(:,:,:,jn) )
         END DO
      ENDIF
9001  FORMAT(3x,i3,1x,a10,3x,l2,3x,l2,3x,l2,3x,l2, 3x, f12.5)
      !
...
!!gm BUG ?   if damping and restart, what's happening ?
        ...
        WHERE( trn(:,:,:,1:jp_bgc) .gt. 0. ) flagRIV = 1.  ! 1 to state where there are rivers (runoff used for mass flux)
        WHERE( trn(:,:,:,1:jp_bgc) .lt. 0. ) flagOTH = 1.  ! 1 to state where there are other sources (velocity used for mass flux)
        trn(:,:,:,1:jptra) = 0._wp               ! turn off input - now mass flux
        trb(:,:,:,1:jptra) = trn(:,:,:,1:jptra)
        tra(:,:,:,1:jptra) = trn(:,:,:,1:jptra)
        !--- END NB 
        !
        !trb(:,:,:,:) = trn(:,:,:,:)
        !
